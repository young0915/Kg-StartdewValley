...
HRESULT maptool::init()
{
	...
	for (int i = 0; i < 36; i++)
	{
		...
		_springfloor[i]->init("images/맵툴/maptool_tile.bmp", 600, 1300, 12, 26, true, RGB(255, 0, 255));
	}
...
void maptool::render()
{
	...
	for (int i = 0; i < TILEX * TILEY; i++)
	{
		if (CAMERAX - 100 < _tile[i].x && _tile[i].x < CAMERAX + WINSIZEX + 100 && CAMERAY - 100 < _tile[i].y&& _tile[i].y < CAMERAY + WINSIZEY + 100)
		{
			...
			{
				IMAGEMANAGER->frameRender("맵툴바닥", getMemDC(), _tile[i].rc.left, _tile[i].rc.top, _tile[i].terrainFrameX, _tile[i].terrainFrameY);
